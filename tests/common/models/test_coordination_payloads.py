...
def test_coalition_and_resolution():
    inv = CoalitionInvite(
        threat_id="t1", segments=[Segment.SERVER, Segment.INTERNAL], required_roles=["ACA", "RCA"]
    )
    rec = CoalitionRecord(
        coalition_id=inv.coalition_id,
        members=["ACA:seg1", "RCA:seg1"],
        lead_rca="RCA:seg1",
        threat_id="t1",
    )
...
def test_error_payloads():
    f = Failure(
        ...
        receiver="RAA:1",
        reason="TIMEOUT",
        failed_action="QUARANTINE",
        description="vote deadline exceeded",
